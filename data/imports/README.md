...
Operator-owned pages with a public Mariana Tek embed receive a bounded card pass after the frame becomes visible. The adapter reads only stable product IDs, card headings, dedicated price nodes, and the visible location label. Background JSON for the same product is deduplicated, per-class arithmetic is ignored, gift cards are excluded, and private or special-purpose sessions are retained as non-ordinary review evidence.

Bookee storefronts receive the same bounded treatment. The renderer preserves only the strict read-only `#/pricing/r/{resource}/loc/{location}` fragment, combines it with an operator-linked `onbookee.com` tenant host without guessing IDs, and ties each amount to one visible `service_group_id` card, heading, price node, allowance, term, section, and location. Some Bookee tenants return an empty AWS WAF shell to fresh headless browsers even though the ordinary public browser renders the catalog. Those attempts are recorded as `platform-security-check`; the bot never copies or bypasses WAF tokens. `public-browser-captures.json` is the deterministic review-only fallback for such cases. It accepts only compact card fields from an exact reviewed operator/storefront route, stores no HTML or browser state, preserves the original capture date and content hash, and still requires the ordinary catalog-review workflow before publication.

Mindbody links receive an additional bounded recovery pass. Any reviewed `clients.mindbodyonline.com` deep link with a numeric `studioid` is normalized to that business's unauthenticated Services route, and public numeric `optTG` category IDs are followed with memberships/access categories first and a twelve-category cap. The renderer recognizes the public recurring-plan tab under both current labels, `Contracts` and `Monthly Memberships`; all other link labels fail closed. Account, gift-card, cart, checkout, and nonnumeric actions are excluded. A blank HTTP-200 shell whose only action is `mb.sessionHelpers.resetSession()` is recorded as `identity-session-reset-required`, because completing that flow would submit an identity form; the bot never does so. When browser automation cannot reproduce an otherwise public catalog, `public-browser-captures.json` accepts only compact contract/row fields from an exact numeric `studioid` Services URL with no checkout parameters. Rendered Mindbody cards become monthly only when the card explicitly says monthly, per month, every month, recurring, or auto-renewing; first-month promotions are stored as separate candidates. Fixed-duration labels such as “3 Month Special” remain one-time fixed-term offers and cannot inflate the neutral monthly comparison.
