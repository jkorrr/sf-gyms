...
The current static site uses MapLibre GL JS with OpenFreeMap's public `liberty` style (`https://tiles.openfreemap.org/styles/liberty`). Gym locations are rendered as small HTML markers on top of the vector map. Clicking a card or marker selects the same gym, the map fits the current filtered set, and a selected marker flies into view. OpenStreetMap attribution is always displayed through MapLibre's attribution control and the page footer.

The initial San Francisco directory is generated by `data/imports/import_osm_sf_gyms.py`. It queries named OSM-tagged `fitness_centre`, `sports_centre`, `gym`, and relevant sport features through Overpass, normalizes nodes/ways/relations, preserves each OSM source URL, and writes a committed snapshot. This is a lawful, repeatable public-data import—not an exhaustive claim about every commercial gym. Business prices and hours are left unknown until a permitted source, owner claim, or user report verifies them.
...
6. avoid bulk tile downloads and keep visible OpenStreetMap/OpenFreeMap attribution;
