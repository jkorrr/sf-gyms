...
## Map and directory behavior

The current static site uses MapLibre GL JS with OpenFreeMap's public `liberty` style (`https://tiles.openfreemap.org/styles/liberty`). Gym locations are rendered as 20px HTML markers on top of the vector map. Clicking a preview card or marker selects the same gym, the map fits the current filtered set, and a selected marker flies into view. The UI is map-first: it does not render every gym in a permanent sidebar; it shows a small preview strip and a selected detail card instead. A neighborhood filter narrows the dataset and causes the map to refit to that neighborhood. If the public vector style does not finish loading, the map switches to OpenStreetMap raster streets and lets the user switch providers manually. OpenStreetMap attribution is always displayed through MapLibre's attribution control and the page footer.

The initial San Francisco directory is generated by `data/imports/import_osm_sf_gyms.py`. It queries named OSM-tagged `fitness_centre`, `sports_centre`, `gym`, and relevant sport features through Overpass, normalizes nodes/ways/relations, preserves each OSM source URL, assigns approximate neighborhood labels when OSM has no neighborhood tag, and writes a committed snapshot. This is a lawful, repeatable public-data import - not an exhaustive claim about every commercial gym. Business prices and hours are left unknown until a permitted source, owner claim, or user report verifies them.

The current price enrichment is a small, reviewable set of official observations in `data/imports/official-price-overrides.json`. The importer joins an observation to a normalized listing by an explicit gym-name match and carries the source URL, observation date, and plan caveat into the static fixture. The UI never presents these as universal rates: it labels them as official observations and links back to the source. A future production importer should use an allowlisted source registry, honor each site's terms and robots requirements, rate-limit requests, record retrieval failures, and preserve price history rather than overwriting old assertions.

The “distance from” control supports browser geolocation without uploading coordinates and a one-shot user-submitted neighborhood/address lookup through Nominatim. It does not implement autocomplete or bulk geocoding. The Nominatim integration should be replaced with a proxied or self-hosted geocoder before traffic grows beyond a small prototype, and must stay within the service's usage policy.

Production map/data requests should:
...
6. avoid bulk tile downloads and keep visible OpenStreetMap/OpenFreeMap attribution; the standard OpenStreetMap tile endpoint is a prototype fallback, not a commitment to support high-volume commercial traffic;
7. keep the tile style URL configurable so the product can move to self-hosted vector tiles or another permitted provider as usage grows.
...
For GitHub Pages, the project-root URL (`/sf-gyms/`) is the callback target. The browser exchanges the single-use code, removes it from the URL before the asynchronous exchange, and rejects an invalid destination unless it is on an allowlist. The PKCE verifier is scoped to the browser tab. Expired, reused, or mismatched codes restart login without logging codes or tokens. Duplicate callbacks therefore do not leave an authorization code in the address bar or create duplicate browser-side initialization.

The static client reads only `NEXT_PUBLIC_SUPABASE_URL` and `NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY` at build time. It accepts current `sb_publishable_...` keys and legacy browser-safe anon JWTs, rejects service-role/secret-shaped values, and validates the URL before creating a client. A missing or invalid pair falls back to the read-only directory with a user-facing setup message; it must not crash the static page. `createClient` initialization is also guarded so malformed build-time configuration cannot prevent public browsing. Public build variables may appear in the browser bundle by design; service-role keys, database credentials, and Google OAuth client secrets may not.

The current static prototype keeps saved gym IDs in browser storage. Do not claim that this is cloud-synced merely because Supabase Auth is configured. Enable the `saved_gyms` sync only after the imported directory maps every listing to a real Supabase `gym_location_id`, the unique `(user_id, gym_location_id)` constraint is present, and RLS has been tested for cross-user reads and writes. This avoids silently creating unusable rows from OSM fixture IDs.
